...
Due to some slightly non-uniform ways of naming the benchmark tests, a new
standard naming conventions was established.  This scripts is used to
convert the existing benchmark test names contained in the existing results
to conform to the (v2) standard. v2 naming goes into effect on benchmark
results starting 2019-12-05.
            ...
            if match1:
                ...
                response = subprocess.run(
                    ['sed',
                     ...
                     '',
                     '-E', '-e',
                     's~BM_([^\/]*)_multiCore\/([^\/]*)~BM_\\1\/multiCore\/\\2~g',
                     '-e',
                     's~BM_([^\/]*)_singleCore~BM_\\1\/singleFed~g',
                     file],
                    text=True)
                if response.returncode != 0:
                    logging.warning(
                        'Failed update of benchmark names in {}'.format(file))
                ...
                if match2:
                    ...
                    # Only the 'messageSendResults' results files get
                    # updated here
                    response = subprocess.run(
                        ['sed',
                         '-i',
                         ...
                         '-E',
                         '-e',
                         's~BM_([^\/]*)\/(.*)MultiCore~BM_\\1\/multiCore\/\\2Core~g',
                         '-e',
                         's~BM_([^\/]*)\/singleCore~BM_\\1\/singleFed~g', 
                         file],
                        text=True)
                    if response.returncode != 0:
                        logging.warning(
                            'Failed update of benchmark names in {}'.format(
                                file))
                        logging.warning('    {}'.format(response.stderr))
                    else:
                        logging.info('Updated benchmark names in {}'.format(
                            file))
