...
performance as more runs are done across time.  This
...
def create_benchmark_tracking_report(output_path, meta_bmk_df):
    """This function creates the benchmark tracking report.
...
def grab_header_metadata(meta_bmk_df):
    ...
    header_metadata_str = ''
    # Filtering the dataframe; we want to look at the most common results as
    # a basis for monitoring the performance of HELICS.
...
def add_benchmark_graphs(pdf, output_path):
    ...
    logging.info('added all the graphs to the report')
    return pdf
...
def make_benchmark_track_graphs(meta_bmk_df, output_path):
    ...
    Returns:
        (null)
    """
    # Converting datetime to just date for simplifying the graphs.
    ...
    logging.info('created all the benchmark tracking graphs')
...
def _auto_run(args):
    ...
    create_benchmark_tracking_report(output_path,
                                     meta_bmk_df)
    logging.info('finished creating the benchmark tracking report')
    # Replacing run-names to the correct run-names.
    ...
    # Creating separate graphs for the conversionBenchmark
    ...
    plot1 = df1.sort_values('DATE').hvplot.line(
        'DATE', 'real_time',
        ylabel='real_time (s)', line_width=3,
        by='run_name', rot=90,
        alpha=0.75).opts(
    ...
    logging.info('creating the conversionBenchmark plots.')
    logging.info('finished the execution of this script.')
