...
def plot_echo_msg_cr(dataframe, run_id_list, core_type):
    ...
    for run_id in run_id_list:
        echo_df = dataframe[ (dataframe.benchmark == 'echoMessageBenchmark') & (dataframe.core_type == '{}'.format(core_type)) & (dataframe.run_id == '{}'.format(run_id))]
        echo_msg = echo_df.sort_values('federate_count').hvplot.line(
                'federate_count', 
                'real_time', 
                ylabel='real_time (ns)', 
                title='run_ids {}, core_type: {} echoMessageBenchmark: federate_count vs real_time'.format(run_id_list, core_type), 
                alpha=0.5).opts(
                        width=1000, 
                        height=600, 
                        logx=True, 
...
def plot_echo_result_cr(dataframe, run_id_list, core_type):
    ...
    echo_ress = []
    for run_id in run_id_list:
        echo_df = dataframe[ (dataframe.benchmark == 'echoBenchmark') & (dataframe.core_type == '{}'.format(core_type)) & (dataframe.run_id == '{}'.format(run_id))]
        echo_res = echo_df.sort_values('federate_count').hvplot.line(
                'federate_count', 
                'real_time', 
                ylabel='real_time (ns)', 
...
def plot_msg_lookup_cr(dataframe, run_id_list):
    ...
    msg_lookups = []
    for run_id in run_id_list:
        msg_lkp_df = dataframe[ (dataframe.benchmark == 'messageLookupBenchmark') & (dataframe.core_type == 'inproc') & (dataframe.run_id == '{}'.format(run_id))]
        msg_lookup = msg_lkp_df.sort_values('federate_count').hvplot.bar(
                'federate_count', 
                'real_time', 
                ylabel='real_time (ns)', 
                title='run_ids {} core_type: inproc messageLookupBenchmark: federate_count vs real_time'.format(run_id_list),
                alpha=0.5).opts(
                        width=1000, 
                        height=600, 
                        logx=True, 
...
def plot_msg_send_1_cr(dataframe, run_id_list):
    ...
    msg_sends = []
    for run_id in run_id_list:
        msg_snd_df = dataframe[ (dataframe.benchmark == 'messageSendBenchmark') & (dataframe.core_type == 'singleFed') & (dataframe.run_id == '{}'.format(run_id))]
        msg_send = msg_snd_df.sort_values('message_size').hvplot.line(
                'message_size', 
                'real_time', 
                ylabel='real_time (ns)', 
                title='run_ids {} core_type: {} messageSendBenchmark: message_size vs real_time'.format(run_id_list, 'singleFed'),
                alpha=0.5).opts(
                        width=1000, 
                        height=600, 
                        logx=True, 
...
def plot_msg_send_2_cr(dataframe, run_id_list, core_type):
    ...
    msg_cts = []
    for run_id in run_id_list:
        msg_ct_df = dataframe[ (dataframe.benchmark == 'messageSendBenchmark') & (dataframe.core_type == 'singleFed') & (dataframe.run_id == '{}'.format(run_id)) & (dataframe.message_count == 1)]
        msg_count = msg_ct_df.sort_values('message_size').hvplot.line(
                'message_size', 
                'real_time', 
                ylabel='real_time (ns)', 
                title='run_ids {} core_type: {} messageSendBenchmark, message_count = 1: message_size vs real_time'.format(run_id_list, core_type),
                alpha=0.5).opts(
                        width=1000, 
                        height=600, 
                        logx=True, 
...
def plot_msg_send_3_cr(dataframe, run_id_list, core_type):
    ...
    msg_sizes = []
    for run_id in run_id_list:
        msg_sz_df = dataframe[ (dataframe.benchmark == 'messageSendBenchmark') & (dataframe.core_type == 'singleFed') & (dataframe.run_id == '{}'.format(run_id)) & (dataframe.message_size == 1)]
        msg_size = msg_sz_df.sort_values('message_count').hvplot.line(
                'message_count', 
                'real_time', 
                ylabel='real_time (ns)', 
                title='run_ids {} core_type: {} messageSendBenchmark, message_size = 1: message_count vs real_time'.format(run_id_list, core_type),
                alpha=0.5).opts(
                        width=1000, 
                        height=600, 
                        logx=True, 
...
def plot_phold_cr(dataframe, run_id_list, core_type):
    ...
    pholds = []
    for run_id in run_id_list:
        phold_df = dataframe[ (dataframe.benchmark == 'pholdBenchmark') & (dataframe.core_type =='{}'.format(core_type)) & (dataframe.run_id == '{}'.format(run_id))]
        phold = phold_df.sort_values('federate_count').hvplot.line(
                'federate_count', 
                'real_time', 
                ylabel='real_time (ns)', 
...
def plot_ring_cr(dataframe, run_id_list, core_type):
    ...
    rings = []
    for run_id in run_id_list:
        ring_df = dataframe[ (dataframe.benchmark == 'ringBenchmark') & (dataframe.core_type == '{}'.format(core_type)) & (dataframe.run_id == '{}'.format(run_id))]
        ring = ring_df.sort_values('federate_count').hvplot.line(
                'federate_count', 
                'real_time', 
                ylabel='real_time (ns)', 
...
def plot_filter_cr(dataframe, run_id_list):
    ...
    filters = []
    for run_id in run_id_list:
        filter_df = dataframe[ (dataframe.benchmark == 'filterBenchmark') & (dataframe.core_type == 'singleCore') & (dataframe.run_id == '{}'.format(run_id))]
        filtr = filter_df.sort_values('federate_count').hvplot.line(
                'federate_count', 
                'real_time', 
                ylabel='real_time (ns)', 
                title='run_ids {}, core_type: {} filterBenchmark: federate_count vs real_time'.format(run_id_list, 'singleCore'), 
                by='filter_location',
                alpha=0.5).opts(
                        width=1000, 
                        height=600)
...
def plot_src_cr(dataframe, run_id_list, core_type):
    ...
    sources = []
    for run_id in run_id_list:
        filter_df = dataframe[ (dataframe.benchmark == 'filterBenchmark') & (dataframe.core_type == '{}'.format(core_type)) & (dataframe.run_id == '{}'.format(run_id)) & (dataframe.filter_location == 'source')]
        source = filter_df.sort_values('federate_count').hvplot.line(
                'federate_count', 
                'real_time', 
                ylabel='real_time (ns)', 
                title='run_ids {} core_type: {}, filterBenchmark, filter_location {}: federate_count vs real_time'.format(run_id_list, core_type, 'source'),
                alpha=0.5).opts(
                        width=1000, 
                        height=600, 
                        logx=True, 
...
def plot_dest_cr(dataframe, run_id_list, core_type):
    ...
    dests = []
    for run_id in run_id_list:
        filter_df = dataframe[ (dataframe.benchmark == 'filterBenchmark') & (dataframe.core_type == '{}'.format(core_type)) & (dataframe.run_id == '{}'.format(run_id)) & (dataframe.filter_location == 'destination')]
        filter_df = filter_df[ filter_df.run_id == '{}'.format(run_id)]
        dest = filter_df.sort_values('federate_count').hvplot.line(
                'federate_count', 
                'real_time', 
                ylabel='real_time (ns)', 
                title='run_ids {} core_type: {}, filterBenchmark, filter_location {}: federate_count vs real_time'.format(run_id_list, core_type, 'destination'),
                alpha=0.5).opts(
                        width=1000, 
                        height=600, 
                        logx=True, 
...
#    echo_message = plot_echo_msg_cr(meta_bmk_df, ['aUZF6', 'Zu60n'], 'inproc')
#    echo_result = plot_echo_result_cr(meta_bmk_df, ['aUZF6', 'Zu60n'], 'inproc')
...
#                 destination]
#    
#    save_plots_cr(plot_list, ['aUZF6', 'Zu60n'], 'inproc')
#    
#main()